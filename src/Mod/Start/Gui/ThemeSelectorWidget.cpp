...
void ThemeSelectorWidget::setupButtons(QBoxLayout* layout)
{
    ...
    std::map<Theme, QString> themeMap {
        {Theme::Classic, tr("CoreCAD Classic")},
        {Theme::Dark, tr("CoreCAD Dark")},
        {Theme::Light, tr("CoreCAD Light")}
    };
    ...
    for (const auto& theme : themeMap) {
        ...
        if (theme.first == Theme::Classic && styleSheetName.isEmpty()) {
            ...
            && styleSheetName.contains(QLatin1String("CoreCAD Light"), Qt::CaseSensitivity::CaseInsensitive)
        ) {
            button->setChecked(true);
        }
        else if (
            theme.first == Theme::Dark
            && styleSheetName.contains(QLatin1String("CoreCAD Dark"), Qt::CaseSensitivity::CaseInsensitive)
        ) {
            button->setChecked(true);
        }
...
void ThemeSelectorWidget::themeChanged(Theme newTheme)
{
    ...
    switch (newTheme) {
        case Theme::Classic:
            prefPackManager->apply("CoreCAD Classic");
            break;
        case Theme::Dark:
...
void ThemeSelectorWidget::retranslateUi()
{
    ...
    _buttons[static_cast<int>(Theme::Dark)]->setText(tr("CoreCAD Dark", "Visual theme name"));
    _buttons[static_cast<int>(Theme::Light)]->setText(tr("CoreCAD Light", "Visual theme name"));
    _buttons[static_cast<int>(Theme::Classic)]->setText(tr("CoreCAD Classic", "Visual theme name"));
}
