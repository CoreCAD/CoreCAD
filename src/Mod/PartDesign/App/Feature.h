...
public:
    ...
    /// Cruth #3: which copy of a pattern this step builds on, by the copy's ordinal (its place
    /// in the pattern's transform sequence, the key the break-out skip-list uses, §5.6), or -1
    /// for the base feature's whole output. A step added to one copy's body starts from that
    /// copy alone, as a fillet starts from the edges it names; the other copies' bodies are
    /// untouched. When the copy no longer exists the step fails rather than fall back to the
    /// whole pattern.
    ...
    Part::TopoShape narrowToBaseInstance(
