    ...
    if (it != config.end()) {
        ...
        if (auto url_itr = config.find("BuildRepositoryURL"); url_itr != config.end()) {
            ...
            if (url == QStringLiteral("Unknown")) {
                url = QStringLiteral("https://github.com/CoreCAD/CoreCAD");  // Just take a guess
            }
...
void AboutDialog::showCredits()
{
    ...
    QString creditsHTML
        = QStringLiteral("<html><body><h1>%1</h1><p>%2</p><h2>%3</h2><ul>")
              ...
              .arg(tr("CoreCAD is built on FreeCAD, which would not be possible without the contributions of:"))
