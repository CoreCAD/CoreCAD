...
| ⬜ Todo | **Sketch creation cycle guard:** when "Create sketch on face" is invoked and the support object is a transitive dependency of the active Body, the sketch is silently placed inside that Body — creating a DAG cycle and a recompute failure with no user-friendly error. Fix: before placing the sketch, check if the support is an ancestor of the active Body; if so, auto-create a new Body or show a clear prompt. See `.local/compact-phase3.md` Deferred Items §3. |
