...
| 🟡 | 2 | **Tree derives from chain:** `ViewProviderBody::claimChildren` override added — walks the `BaseFeature` chain from Tip (pipeline order), nests profiles under their feature, surfaces auxiliary objects (Origin/datums/orphan sketches) so nothing vanishes. Build clean; App 464/ Part 274/ PartDesign 8 tests green; verified live (MCP): normal body tree unchanged `[Origin,Pad1,Pad2]`, **and a de-owned Tip (Pad2 pulled out of `Group`) now still appears in the tree** — old group-based code would have dropped it. **Commit pending.** |
