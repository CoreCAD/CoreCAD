...
| ◐ | 4 | **Delete semantics by reference.** *Feature delete DONE* (commit `d2ae2c90de`): `Body::removeObject` flag-gated path (`removeObjectDeowned`) heals the `BaseFeature` chain directly — chain successor (found by `getNextSolidFeatureByChain` doc scan) relinks to the removed feature's base, Tip retreats, Group never consulted; `ViewProvider::onDelete` resolves the Body via `_Body`/`getFeatureBody` not Group-based `findBodyOf`. App 464/Part 274/PartDesign 8 green (flag off unchanged). Live: middle-delete on a de-owned Pad1→Pad2→Pad3 body heals Pad3.Base→Pad1, Tip stays Pad3, no cascade, recomputes to vol 1012. *Remaining:* (a) **body delete** — `removeObjectsFromDocument` cascades over `Group`, which is empty for a de-owned body → must delete exclusively-owned features (not referenced ones) by chain instead; (b) **body-eviction regression** (deleting a cross-body feature evicts Body from its Part). |
